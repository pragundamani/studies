Canvas is very powerful tool that can be used for many things such as:
1. Linking together multiple notes that interlinked in a large space
2. Creating a sticky note board or notes board 
...
**Ribbon:**
- In the ribbon on the left, select **Create new canvas** (![[canvas.png]]) to create a canvas in the same folder as the active file.

**Command palette:**
1. Open the [Command palette](https://help.obsidian.md/Plugins/Command+palette) `CTRL+P` or `CMD+P`. 
...
Or, you can add them to the canvas by dragging the file from the.
